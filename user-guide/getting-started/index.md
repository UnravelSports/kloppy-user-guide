...
This chapter is here to help you get started quickly with kloppy. It gives a 10-minute introduction on how kloppy can be used to [load](#loading-data), [filter](#filtering-data), [transform](#transforming-data), and [export](#exporting-data) soccer match data. If you're already familiar with kloppy, feel free to skip ahead to the [next chapters](installation.md) for a more in-depth discussion of all functionality.
...
In soccer analytics, data typically comes in two main formats: [event data](/user-guide/concepts/event-data) and [tracking data](/user-guide/concepts/tracking-data). Each format has unique advantages and they are often used together.
...
In this section, we illustrate how to load a dataset in this standardized format. We will illustrate this using a sample of [publicly available data provided by Sportec](https://www.nature.com/articles/s41597-025-04505-y). For examples on loading data from other data providers, see the [Loading Data](/user-guide/loading-data) section of the user guide.
...
pitch.lines(
    ...
    1-assist_event.receiver_coordinates.x, 1-assist_event.receiver_coordinates.y,
    lw=5, transparent=True, comet=True, cmap='jet', zorder=1.2,
...
# title
axs['title'].text(
    0.5, 0.5, f'{goal_event.time} - Goal by {goal_event.player} ({goal_event.team})',
...
This will create a [`TrackingDataset`][kloppy.domain.TrackingDataset], which contains a sequence of [`Frame`][kloppy.domain.Frame] entities. Each frame has a [`ball_coordinates`][kloppy.domain.Frame.ball_coordinates] attribute that stores the coordinates of the ball and a [`players_coordinates`][kloppy.domain.Frame.players_coordinates] attribute that stores the coordinates of each player.
...
for player, player_coordinates in first_frame.players_coordinates.items():
      print(f"\t{player} ({player.team}): {player_coordinates}")
```
...
goal_frame = tracking_dataset.find(lambda f: int(f.frame_id) == int(matched_frame_idx))
```
...
for player, player_coordinates in goal_frame.players_coordinates.items():
    ...
    elif player.position == PositionType.Goalkeeper:
...
pattern = (
    ...
    # ... that are followed by N - 1 successful passes by the same team
    + pm.match_pass(
        success=True,
...
!!! note
    ...
    An error will be thrown if you do not have pandas or polars installed.
...
To support this popular data format, kloppy provides a [`CodeDataset`][kloppy.domain.CodeDataset]. You can use kloppy to load SportsCode XML files, but perhaps more interestingly, you can also generate these files from another dataset allowing you to automatically create playlists from event and/or tracking data that can be used by the video analyst. We will illustrate this by creating a playlist with all shots.
